 ...
 * generating and sleeps once the picture is in (or before it starts).
 *
 * Props:
...
const CSS = `
.pui-reveal { position: relative; display: block; width: 100%; max-width: 100%; box-sizing: border-box; overflow: hidden; border-radius: 12px; background: #f5f5f4; aspect-ratio: var(--pui-aspect, 4 / 3); }
...
export default function ImageReveal({
  ...
  const sim = useRef({
    target: 0, p: 0, pts: [], byOrder: [], cols: 0, rows: 0, W: 1, H: 1, settled: false, generating: false,
    t: 0, last: 0,
  });
  ...
  useEffect(() => {
    ...
    s.rows = Math.max(4, Math.round(s.H / cell));
    ...
    function draw() {
      ...
      const base = 0.3 + p * 0.2;
      for (const q of s.pts) {
        if (q.lock) continue;
        const a = base + Math.sin(now * 1.7 + q.seed * 4) * 0.1;
        ctx.fillStyle = `rgba(120,113,108,${a})`;
        ctx.beginPath();
        ctx.arc(q.x, q.y, q.g, 0, Math.PI * 2);
        ctx.fill();
      }
      // landed: grow into place with a small overshoot, then sit on the grid
      for (const q of s.pts) {
        if (!q.lock) continue;
        const age = q.home ? 1 : Math.min(1, (now - q.lockAt) / 0.42);
        const k = age < 1 ? 0.5 + 0.7 * age - 0.2 * age * age : 1; // 0.5 → 1.05 → 1
        ctx.fillStyle = `rgba(120,113,108,${0.45 + 0.4 * age})`;
        ctx.beginPath();
